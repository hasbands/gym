@extends('template-web.layout')

@section('content')
    <div class="container py-5" style="margin-top: 100px;">
        <div class="row justify-content-center">
            <div class="col-12">
                <div class="card border-0 shadow">
                    <div class="card-body p-4">
                        <h2 class="fw-bold mb-4 text-center">Profil</h2>
                        
                        <div class="row">
                            <div class="col-md-4 text-center mb-4">
                                <div class="mb-3">
                                    @if(auth()->user()->foto)
                                        <img src="{{ asset('images/profil/' . auth()->user()->foto) }}" class="rounded-circle" style="width: 150px; height: 150px; object-fit: cover;">
                                    @else
                                        <img src="data:image/png;base64," class="rounded-circle" style="width: 150px; height: 150px; object-fit: cover;">
                                    @endif
                                </div>
                                <h5 class="fw-bold">{{ auth()->user()->nama }}</h5>
                                <p class="text-muted">{{ auth()->user()->role }}</p>
                            </div>
                            
                            <div class="col-md-4">
                                <form action="{{ route('web.profil.update', auth()->user()->id) }}" method="POST" enctype="multipart/form-data">
                                    @csrf
                                    @method('PUT')
                                    
                                    <div class="mb-3">
                                        <label class="form-label">Nama Lengkap</label>
                                        <input type="text" class="form-control" name="nama" value="{{ auth()->user()->nama }}" required>
                                    </div>
                                    
                                    <div class="mb-3">
                                        <label class="form-label">Email</label>
                                        <input type="email" class="form-control" name="email" value="{{ auth()->user()->email }}" required>
                                    </div>
                                    
                                    <div class="mb-3">
                                        <label class="form-label">Username</label>
                                        <input type="text" class="form-control" name="username" value="{{ auth()->user()->username }}" required>
                                    </div>
                                    
                                   
                                    
                                    <div class="mb-3">
                                        <label class="form-label">Nomor WhatsApp</label>
                                        <input type="text" class="form-control" name="no_whatsapp" value="{{ auth()->user()->no_whatsapp }}">
                                    </div>
                                    
                                    <div class="mb-3">
                                        <label class="form-label">Alamat</label>
                                        <textarea class="form-control" name="alamat" rows="3">{{ auth()->user()->alamat }}</textarea>
                                    </div>
                                    
                                    <div class="mb-3">
                                        <label class="form-label">Foto Profil</label>
                                        <input type="file" class="form-control" name="foto" accept="image/*">
                                    </div>
                                    
                                    <div class="mb-3">
                                        <label class="form-label">Password Baru (Kosongkan jika tidak ingin mengubah)</label>
                                        <input type="password" class="form-control" name="password">
                                    </div>
                                    <div class="mb-3">
                                        <label class="form-label">Konfirmasi Password Baru</label>
                                        <input type="password" class="form-control" name="password_confirmation">
                                    </div>
                                    <div class="text-end">
                                        <button type="submit" class="btn btn-primary">Simpan Perubahan</button>
                                    </div>
                                </form>
                            </div>

                            <div class="col-md-4">
                                <div class="card border-0 shadow">
                                    <div class="card-body">
                                        <h5 class="card-title fw-bold mb-4">Detail Membership</h5>
                                     
                                        @if($membership)
                                            @php
                                                $startDate = \Carbon\Carbon::parse($membership->mulai);
                                                $endDate = \Carbon\Carbon::parse($membership->selesai);
                                                $today = \Carbon\Carbon::now();
                                                $totalDays = $startDate->diffInDays($endDate);
                                                $daysUsed = $startDate->diffInDays($today);
                                                $daysRemaining = $endDate->diffInDays($today);
                                                $progressPercentage = min(($daysUsed / $totalDays) * 100, 100);
                                            @endphp

                                            <div class="mb-4">
                                                <div class="d-flex justify-content-between align-items-center mb-2">
                                                    <span class="text-muted">Progress Membership</span>
                                                    <span class="fw-bold">{{ number_format($progressPercentage, 1) }}%</span>
                                                </div>
                                                <div class="progress" style="height: 10px;">
                                                    <div class="progress-bar bg-success" role="progressbar" 
                                                         style="width: {{ $progressPercentage }}%" 
                                                         aria-valuenow="{{ $progressPercentage }}" 
                                                         aria-valuemin="0" 
                                                         aria-valuemax="100"></div>
                                                </div>
                                            </div>

                                            <div class="row mb-3">
                                                <div class="col-6">
                                                    <div class="card bg-light">
                                                        <div class="card-body p-2 text-center">
                                                            <small class="text-muted d-block">Hari Terpakai</small>
                                                            <h5 class="mb-0 fw-bold">{{ $daysUsed }} hari</h5>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="col-6">
                                                    <div class="card bg-light">
                                                        <div class="card-body p-2 text-center">
                                                            <small class="text-muted d-block">Sisa Hari</small>
                                                            <h5 class="mb-0 fw-bold">{{ $daysRemaining }} hari</h5>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>

                                            <div class="membership-details">
                                                <div class="d-flex justify-content-between mb-2">
                                                    <span class="text-muted">Status:</span>
                                                    <span class="badge bg-success">Aktif</span>
                                                </div>
                                                <div class="d-flex justify-content-between mb-2">
                                                    <span class="text-muted">Paket:</span>
                                                    <span class="fw-bold">{{ $membership->masterPaket->nama_paket ?? '-' }}</span>
                                                </div>
                                                @if($membership->masterSuplemen)
                                                <div class="d-flex justify-content-between mb-2">
                                                    <span class="text-muted">Suplemen:</span>
                                                    <span class="fw-bold">
                                                        {{ $membership->masterSuplemen->nama_suplemen }}
                                                        ({{ $membership->jumlah_suplemen }} pcs)
                                                    </span>
                                                </div>
                                                @endif
                                                <div class="d-flex justify-content-between mb-2">
                                                    <span class="text-muted">Mulai:</span>
                                                    <span class="fw-bold">{{ \Carbon\Carbon::parse($membership->mulai)->isoFormat('D MMMM Y') }}</span>
                                                </div>
                                                <div class="d-flex justify-content-between mb-2">
                                                    <span class="text-muted">Selesai:</span>
                                                    <span class="fw-bold">{{ \Carbon\Carbon::parse($membership->selesai)->isoFormat('D MMMM Y') }}</span>
                                                </div>
                                                <div class="d-flex justify-content-between mb-2">
                                                    <span class="text-muted">Total Bayar:</span>
                                                    <span class="fw-bold">Rp. {{ number_format($membership->total_bayar, 0, ',', '.') }}</span>
                                                </div>
                                                <div class="d-flex justify-content-between mb-2">
                                                    <a href="{{ route('web.cetak_kartu') }}" target="_blank" class="btn btn-primary w-100">
                                                        <i class="fas fa-id-card me-2"></i>
                                                        Cetak Kartu Member
                                                    </a>
                                                </div>
                                                <div class="d-flex justify-content-between mb-2">
                                                    <a href="{{ route('generateqr') }}" target="_blank" class="btn btn-primary w-100">
                                                        <i class="fas fa-qrcode me-2"></i>
                                                        Generate QR Code
                                                    </a>
                                                </div>
                                            </div>
                                        @else
                                            <div class="alert alert-warning">
                                                <i class="fas fa-exclamation-circle me-2"></i>
                                                Anda belum memiliki membership aktif. <a href="{{ route('web.listpaket') }}" class="alert-link">Klik disini</a> untuk berlangganan dan mendapatkan akses penuh ke fasilitas gym.
                                            </div>
                                        @endif
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
@endsection
